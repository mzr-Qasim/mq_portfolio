  ...
  $(function() {
    ...
    ifExists('#contact .contact-section', function() { 
      $('#contact .contact-section .contact-form').on('submit', function(event) {
        ...
        setTimeout(function() {
          // Send form data using AJAX to the Django server
          $.ajax({
            url: form.attr('action'),  // The action URL (Django view)
            type: 'POST',
            ...
            error: function() {
              ...
              submitBtn.removeClass('wait').html('Error').addClass('error');
              feedbackEl.addClass('error').html('There was an error with the request.').fadeIn(200);
            }
          });
        }, 1000);
      });
